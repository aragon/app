...
- **Still-dangling names in kit `.d.ts`, upstream-path gap, not fixed here.** The base extractor never populates `prelude`, so kit-owned type references (`IInputContainerAlert`, `IButtonBaseProps`, `ITagProps`) and kit-internal names (`Container`, `Root`, `Item`, `Structure`, `Skeleton`, `Config`, `Breakpoint`) are emitted unresolved. They parse — which is all the converter's `[DTS_PARSE]` checks — but do not resolve for a consumer. Generic parameters (`TFormData`, `TMeta`, `TStepId`, `TAction`) are correctly bound by the emitted `generics` clause and are NOT part of this. `refresh.test.mjs` covers the React class of this bug across every emitted file; the kit-owned class needs a kit-side type closure like the App one.
