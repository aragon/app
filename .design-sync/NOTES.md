...
- `next/*` shims at `apps/app/src/node_modules/next/` (dynamic→null, image→img, link→a) — nearest-node_modules wins for files under src/; regenerated by `build-css.mjs`. The real `next` package is unbundlable (server internals).
