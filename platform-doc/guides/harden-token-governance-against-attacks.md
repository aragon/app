...
status: draft
source: synthesis of existing platform entries and their protocol-doc links (2026-08-07, see log.md); no new source material — every claim traces to the linked pages + supported-chain reconciliation (2026-09-10, app@d1fa9970; see log.md) + product-owner briefings (2026-09-11, see log.md)
---
...
- **Raise the proposer's required stake.** Ask Aragon to compose an already-deployed condition contract into the advanced process's creation rule — for example the above-a-token-threshold `VotingPowerCondition`, which gates creation on token balance ([existing conditions](../governance/proposal-creation.md#existing-conditions-composing-an-already-deployed-condition)).
...
Work with Aragon to express each safeguard as its own stage of one [staged proposal](../governance/proposal.md#staged-proposals) pipeline. [Add a multisig gate to an advanced governance process](./multisigs-in-advanced-governance.md) helps prepare the multisig configuration for that setup.
...
Prefer a stage over a direct permission grant. Granting creation or execution directly to a Safe works, but it keeps that role out of the [proposal page](../governance/proposal.md#voting)'s stage view — real, yet invisible to the people the safeguards protect — and the creation variant buries the proposal's real actions in nested calldata that Safe signers cannot review with the app's action presentation ([stages over direct permission grants](../governance/multisig-gates.md#stages-over-direct-permission-grants)).

## 7. Review what each proposal actually executes

Design-time safeguards still depend on reviewers reading the proposal in front of them. The [action builder](../application/action-builder.md) makes the exact actions legible, and [action simulation](../application/action-simulation.md) tests the permission and action-execution path — whether the intended caller will be permitted to execute and whether the actions succeed — during creation and on any proposal in a non-terminal state, on [chains with simulation available](../application/supported-chains.md).
