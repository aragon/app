...
status: draft
source: product-owner briefings (2026-07-29, see log.md) + basic-action-view audit (2026-09-10, app@f8bf9e87 + installed @aragon/gov-ui-kit@2.11.2; see log.md) + removal-alert verification (2026-09-13, app@adad67873c8f9dd75e3ed340b70df3e985ae3557; see log.md) + product-owner editorial feedback (2026-09-13, see log.md)
---

# Control availability

Hiding a control, showing it disabled, or showing it enabled and responding at activation are different product messages, not interchangeable implementation choices. Each treatment makes a different promise about whether a capability exists, belongs in the current context, and can succeed. Choose the treatment from the user's reasonable expectation, following [every element makes a claim](./principles/every-element-makes-a-claim.md).
...
**Communication is not enforcement.** Hidden and disabled controls only describe the interface's current understanding. Authorization must still be enforced by the authoritative backend or onchain [permission system](../../protocol-doc/core/permissions.md); hiding a control is never a security boundary.
...
- **Disabled:** when the [action views](../../application/action-builder.md) has more than one supported representation, its menu retains all three Basic/Decoded/Raw options and disables unsupported levels. With only Raw available, the menu is hidden.
...
- **Warned:** [Admin removal](../../governance/admin-flow.md#removal-boundary) asks the operator to consider the loss of admin authority before continuing the handover to governance.
- **Hidden:** when a governance token does not support `delegate(address)`, the [token panel](../../governance/token-panel.md) and the other member surfaces omit their delegation controls, since the token gives no one a reason to expect delegating.
