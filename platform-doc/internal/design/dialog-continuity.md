...
status: draft
source: app interaction-principles inventory against app@122f1bd1 + product-owner review (2026-08-05, see log.md) + product-owner briefings (2026-09-11, see log.md)
---
...
Preserve useful input when returning to the parent, and apply the child result to the part that requested it. If the new decision invalidates the parent, close or replace it explicitly and explain the resulting state.

This pattern governs the relationship between dialogs. The [dialog taxonomy](./dialog-taxonomy.md) still determines which dialog kind fits, and the [wizard pattern](../../application/wizard.md) governs a wizard's own steps.
