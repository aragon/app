...
Scoped authority is a product rule for preserving separation of powers as an account's authority is composed and changed. It uses OSx [permission triples and conditions](../protocol-doc/core/permissions.md). For the actors and call boundaries, see [Authorization and execution](./authorization-and-execution.md); for the concrete Aragon routes, see [OSx authorization paths](./osx-authorization-paths.md).
...
For caller paths, permission checks, and ROOT administration, see [OSx authorization paths](./osx-authorization-paths.md).
...
Scoped authority puts an Execute condition on that grant so the executor may forward only the selected calls. In OSx, an [execute selector condition](../protocol-doc/helpers/condition-library/execute-selector-condition.md) evaluates every action inside `DAO.execute` against configured selector and native-transfer allowances; see [OSx authorization paths](./osx-authorization-paths.md#trace-a-standard-plugin-execution) for the exact call cases. The batch passes only when every action passes. The configured scope can keep a process's authority aligned with its assigned responsibility.
...
In OSx, the DAO combines the organization's shared permission authority, general-purpose execution account, stable identity, and treasury. Separate governance plugins can receive independently scoped Execute grants into that account, so several processes can hold distinct powers over one treasury. The [authorization model](./authorization-and-execution.md#shared-authority-and-execution-custody) compares shared authority and asset custody across OSx and OpenZeppelin arrangements.
...
The [governance designer](../governance/governance-designer.md#choosing-authorized-actions) sets a new process's initial scope: **Any action** gives it a bare Execute grant, while **Specific actions** creates a selector-scoped grant. The [action builder](../application/action-builder.md) composes governed change actions. [Permission Viewer](./permission-viewer.md) presents the account-wide indexed grants and conditions, while the process details page presents the selected authorized actions for one process.
