...
export type ButtonIconProps = Omit<ButtonBaseProps, 'label' | 'iconRight' | 'iconLeft' | 'iconOnly'> & {
    bgWhite?: boolean;
...
export const ButtonIcon = React.forwardRef<HTMLButtonElement, ButtonIconProps>(
    ({ bgWhite = false, icon, isActive = false, mode = 'primary', size = 'medium', ...props }, ref) => {
        return (
            <StyledButton
                ref={ref}
                {...props}
...
const StyledButton = styled(ButtonBase).attrs<StyledButtonProps>(({ bgWhite, isActive, mode, size = 'medium' }) => {
    ...
    switch (mode) {
        case 'secondary':
            className = `${bgWhite ? 'bg-ui-50 disabled:bg-ui-50' : 'bg-ui-0 disabled:bg-ui-100'} ${
