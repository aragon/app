...
export const CheckboxSimple: React.FC<CheckboxSimpleProps> = ({
    ...
    return (
        <Container data-testid="checkboxSimple" disabled={disabled} state={state} iconLeft={iconLeft} onClick={onClick}>
...
const Container = styled.div.attrs<ContainerTypes>(({ disabled, state, iconLeft }) => ({
    className: `flex w-max space-x-1.5 items-center ${!iconLeft && 'flex-row-reverse space-x-reverse'} ${
        disabled
            ? 'text-ui-300'
            : `cursor-pointer hover:text-primary-500 ${state !== 'default' ? 'text-primary-500' : 'text-ui-600'}`
    }`,
}))<ContainerTypes>``;
