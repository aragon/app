...
export const CardTransfer: React.FC<CardTransferProps> = ({ to, from, toLabel, fromLabel, bgWhite = false }) => {
    return (
...
const Card: React.FC<CardProps> = ({ label, copy, bgWhite }) => {
    return (
        <Container bgWhite={bgWhite}>
            <Label>{label}</Label>
            <Value isAddress={IsAddress(copy)}>{IsAddress(copy) ? shortenAddress(copy) : copy}</Value>
        </Container>
    );
};
