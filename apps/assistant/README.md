...
2. **1Password**: create vaults `kv_assistant_infra` (`VERCEL_PROJECT_ID`, `VERCEL_ORG_ID`, `VERCEL_TOKEN`, `SENTRY_AUTH_TOKEN`, `SENTRY_ORG`, `SENTRY_PROJECT`) and `kv_assistant_{development,preview,production}` (`AI_GATEWAY_API_KEY`, `LINEAR_API_KEY`, `LINEAR_TEAM_ID`, `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`, `BLOB_READ_WRITE_TOKEN`, `CRON_SECRET`, `ANALYSIS_API_SECRET`, `SENTRY_DSN`). `LINEAR_TEAM_ID` points at the dedicated test team in the development/preview vaults and at the real support team in production. `ANALYSIS_API_SECRET` is a random bearer secret for the server-to-server proposal-analysis endpoint (`POST /analysis/proposal`); the same value goes to the Aragon backend of that environment as `AI_ANALYSIS_ASSISTANT_SECRET`. While the analysis is a proof of concept the development/preview vaults may leave it out: outside production the service falls back to the default in `src/lib/env.ts`, which the backend also defaults to. Production has no fallback. The backend additionally needs `VERCEL_AUTOMATION_BYPASS_SECRET` (as `AI_ANALYSIS_ASSISTANT_BYPASS_SECRET`): every assistant domain sits behind Vercel's bot challenge, which answers a server-to-server call with a 429 checkpoint page unless the request carries `x-vercel-protection-bypass` - the same mechanism the LLM smoke uses. The Sentry token needs project release/file upload access; CI uses the three infrastructure values only to upload source maps.
