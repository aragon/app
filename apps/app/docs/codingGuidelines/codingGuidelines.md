...
- Outside the column (dialogs, the assistant panel, anything portalled to the body) the same variants
  are viewport media queries, as they always were. `body` and `html` declare no container on purpose:
  containment there stops the page background and body-level scroll locks from reaching the canvas
  and the viewport.
