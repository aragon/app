...
- `body` is the fallback `app` container: the assistant panel, dialogs and anything else portalled to
  the document body measure the window, as they always did. Containment on the body switches off the
  propagation of its properties to the canvas and the viewport, so anything meant for the canvas, such
  as the page background, is declared on the root element in `layoutRoot.tsx`, not on `body`.
