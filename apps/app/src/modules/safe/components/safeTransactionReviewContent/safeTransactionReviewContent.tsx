...
export interface ISafeTransactionReviewContentProps {
    /**
    ...
    network: Network;
    /**
     * Service-reported version, when available, checked against the onchain version.
     */
    safeVersion?: string | null;
    /**
     * Narrow description of what the reviewed payload is intended to do.
     */
    intent?: string;
    /**
     * What confirming will cost the owner in wallet interactions, when the caller knows. Rendered
     * in the compact summary above the optional technical details.
     */
    costNote?: string;
    /**
     * Reports whether the payload is misdescribed — a hard-refusal state — so the surface owning
     * the confirmation action can disable it. Fired whenever the verdict changes; a review-only
     * viewer omits it. The verdict is one-way input to the caller: this component never renders an
     * action of its own.
     */
    onGateChange?: (isPayloadMisdescribed: boolean) => void;
}
...
/**
 * The reviewable payload of a Safe transaction: its fields, the device-comparison hashes, every
 * call it executes, and the refusal/disclosure gates that decide whether it can be signed at all.
 *
 * Shared by the actionless account-queue viewer and the proposal transaction dialog so both show
 * one review surface. The confirmation action lives with whoever owns the flow; this component
 * only reports the misdescription verdict through `onGateChange`.
 */
export const SafeTransactionReviewContent: React.FC<
    ISafeTransactionReviewContentProps
> = (props) => {
    const {
        transaction,
        safeAddress,
     ...
     * the call listed below never happens.
     *
     ...
     * delegate call is refused while it is outstanding and if it fails outright. Failure is treated
     * the same way rather than optimistically: the whole batch's effect hangs on this answer.
     */
    ...
    /**
     * Refused rather than disclosed, because in these states what this surface *says* is wrong, not
     * merely incomplete. An unverifiable hash and an undecodable call stay signable on purpose:
     * both are honest about themselves and a gate there would lock owners out of legitimately
     * exotic payloads.
     */
    ...
    useEffect(() => {
        onGateChange?.(isPayloadMisdescribed);
    }, [isPayloadMisdescribed, onGateChange]);

    return (
        <>
            {intent != null && (
                <p className="pb-4 text-neutral-500 text-sm md:text-base">
                    {intent}
                </p>
            )}
            {isHashMismatch && (
            ...
            {isDelegateTargetUnresolved && (
                <AlertInline
                    message={t(`${translationKey}.delegateTargetUnresolved`)}
                    variant="warning"
                />
            )}
            {hasCodelessTarget && (
            ...
            {hasUnrecognisedDelegateTarget && !hasCodelessTarget && (
                <AlertInline
                    message={t(`${translationKey}.unrecognisedDelegateCall`)}
                    variant="warning"
                />
            )}
            <dl className="flex flex-col gap-3 pt-4">
                <div className="flex flex-row items-center justify-between gap-4">
                    <dt className="text-neutral-500 text-sm md:text-base">
                        {t(`${translationKey}.fields.safe`)}
                    </dt>
                    <dd className="text-neutral-800 text-sm md:text-base">
                        <AddressOutput
                            address={safeAddress}
                            href={safeAppAccountUrl({
                                address: safeAddress,
                                network,
                            })}
                            isExternal={true}
            ...
            {costNote != null && (
                <p className="pt-3 text-neutral-500 text-sm md:text-base">
                    {costNote}
                </p>
            )}
            <AlertCard
                className="mt-4"
                message={t(`${translationKey}.verificationRecommendation`)}
                variant="info"
            />
            <div className="mt-4 flex flex-col gap-3 rounded-xl bg-neutral-50 p-4">
                <Collapsible
                    buttonLabelClosed={t(
                        `${translationKey}.detailsToggle.closed`,
                    )}
                    buttonLabelOpened={t(
                        `${translationKey}.detailsToggle.opened`,
                    )}
                    className={
                        detailsOpen ? undefined : '[&>div:last-child]:mt-0'
                    }
                    collapsedPixels={0}
                    isOpen={detailsOpen}
                    onToggle={setDetailsOpen}
                >
                    <div className="flex flex-col gap-3" inert={!detailsOpen}>
                        <dl className="flex flex-col gap-3">
                            ...
                            <SafeTransactionReviewRow
                                label={t(
                                    `${translationKey}.fields.refundReceiver`,
                                )}
                                value={addressUtils.truncateAddress(
                         ...
                         * match. Deliberately never labelled verified: a self-consistent malicious payload
                         * produces hashes that match the device perfectly.
                         */}
                        ...
                        {!isVersionPending &&
                            (domainHash == null || messageHash == null) && (
                                <p className="text-neutral-500 text-sm">
                                    {t(
                                        `${translationKey}.hashComparisonPartial`,
                                    )}
                                </p>
                            )}
                         ...
                         * make the comparison pass by construction. */}
                                ...
                                >
                                    <div className="flex flex-row items-center gap-2">
                                        ...
                                        <span className="truncate text-neutral-800 text-sm md:text-base">
                                            ...
                                            {localNames[index] ||
                                                decoded?.[index]?.inputData
                                                    ?.function ||
                                                decoded?.[index]?.type ||
                                                t(
                                                    `${translationKey}.unknownAction`,
                                                )}
                                        </span>
                                    </div>
                                    {call.operation === 1 &&
...
const SafeTransactionReviewHash: React.FC<{
    ...
    return (
        <div className="flex flex-col gap-1">
            <dt className="text-neutral-500 text-sm md:text-base">{label}</dt>
            <dd className="break-all font-mono text-neutral-800 text-xs">
                {value}
            </dd>
        </div>
    );
