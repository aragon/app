...
import {
    ...
    type IDaoSettingsPageClientProps,
} from './daoSettingsPageClient';
...
describe('<DaoSettingsPageClient /> component', () => {
    ...
    it('renders the page title', () => {
        ...
        expect(
            screen.getByText(/daoSettingsPage.main.title/),
        ).toBeInTheDocument();
    });
    ...
    it('renders the dao settings, info and contract update components', () => {
        ...
        render(createTestComponent());
        expect(
            screen.getByText(/daoSettingsPage.main.settingsInfoTitle/),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/daoSettingsPage.aside.versionInfoTitle/),
        ).toBeInTheDocument();
        ...
        const permissionsLink = screen.getByRole('link', {
            name: /daoSettingsInfo.permissionsLink/,
        });
        expect(permissionsLink).toHaveAttribute(
            'href',
            '/dao/ethereum-mainnet/1234/permissions',
        );
        const versionCard = permissionsLink.closest('div.flex.shrink-0');
        expect(versionCard).toContainElement(permissionsLink);
    });
    ...
    it('renders the governance processes of the DAO', () => {
        ...
        render(createTestComponent());
        expect(
            screen.getByText(/daoSettingsPage.main.governanceInfoTitle/),
        ).toBeInTheDocument();
        expect(
            screen.getByText(plugins[0].meta.description!),
        ).toBeInTheDocument();
        expect(
            screen.getByText(plugins[1].meta.description!),
        ).toBeInTheDocument();
    });
    ...
    it('does not render governance processes section when dao has no supported plugins', () => {
        ...
        render(createTestComponent());
        expect(
            screen.queryByText(/daoSettingsPage.main.governanceInfoTitle/),
        ).not.toBeInTheDocument();
    });
    ...
    it('returns null when DAO cannot be fetched', () => {
        ...
        render(createTestComponent());
        expect(
            screen.queryByText(/daoSettingsPage.main.title/),
        ).not.toBeInTheDocument();
    });
});
