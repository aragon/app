...
/**
 * Hardcoded workspaces seeded into the mocked workspace registry until a real workspace registry exists.
 *
 * There is no backend endpoint for workspaces yet, so `workspaceService` resolves them from this map merged with the
 * workspaces stored on local storage instead of issuing an HTTP request. The `useMocks` fetch interceptor is not
 * usable here because it only patches `global.fetch` from the client-side providers.
 *
