...
describe('daoExplorer service', () => {
    ...
    it('getDaoList fetches a list of DAOs', async () => {
        const daos = [
            generateDao({ address: '0x123' }),
            generateDao({ address: '0x456' }),
        ];
        const params = {
    ...
    it('getDaoListByMember fetches a paginated list of DAOs for a given member address', async () => {
        ...
        const result =
            await daoExplorerService.getDaoListByMemberAddress(params);

        expect(requestSpy).toHaveBeenCalledWith(
            daoExplorerService['urls'].daoListByMemberAddress,
            params,
        );
