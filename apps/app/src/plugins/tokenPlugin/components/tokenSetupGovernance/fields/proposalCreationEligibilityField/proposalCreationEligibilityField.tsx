...
export const ProposalCreationEligibilityField: React.FC<
    IProposalCreationEligibilityFieldProps
> = (props) => {
    ...
    const { value, onChange, ...field } = useFormField<
        ITokenSetupGovernanceForm,
    ...
    return (
        <Card className="flex flex-col gap-6 border border-neutral-100 p-6 shadow-neutral-sm">
            <RadioGroup
                className="w-full"
                helpText={t(
                    'app.plugins.token.tokenSetupGovernance.eligibleField.helpText',
                )}
                label={t(
                    'app.plugins.token.tokenSetupGovernance.eligibleField.label',
                )}
            ...
            {radioValue === 'members' && (
                <InputNumber
                    ...
                    className="w-full"
                    helpText={t(
                        'app.plugins.token.tokenSetupGovernance.minVotingPower.helpText',
                    )}
                    onChange={onChange}
