...
describe('useGovernanceToken hook', () => {
    ...
    const useDelegationCheckSpy = jest.spyOn(
        useGovernanceTokenDelegationCheck,
        'useGovernanceTokenDelegationCheck',
    );
    const useErc20CheckSpy = jest.spyOn(
        useGovernanceTokenErc20Check,
        'useGovernanceTokenErc20Check',
    );
    const useVotesCheckSpy = jest.spyOn(
        useGovernanceTokenVotesCheck,
        'useGovernanceTokenVotesCheck',
    );
    ...
    it('does not trigger delegation and votes checks if ERC20 check fails', () => {
        ...
        const { result } = renderHook(() =>
            useGovernanceToken({ address: '0x123', chainId: 123 }),
        );
        ...
        expect(useDelegationCheckSpy).toHaveBeenCalledWith(
            expect.objectContaining({ enabled: false }),
        );
        expect(useVotesCheckSpy).toHaveBeenCalledWith(
            expect.objectContaining({ enabled: false }),
        );
    });
    ...
    it('returns fallback token when ERC20 check succeed but token data fetch does not', () => {
        ...
        const { result } = renderHook(() =>
            useGovernanceToken({ address: '0x123', chainId: 1 }),
        );
