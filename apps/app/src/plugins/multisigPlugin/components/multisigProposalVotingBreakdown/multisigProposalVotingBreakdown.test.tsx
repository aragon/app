...
describe('<MultisigProposalVotingBreakdown /> component', () => {
    const createTestComponent = (
        props?: Partial<IMultisigProposalVotingBreakdownProps>,
    ) => {
    ...
    it('correctly displays the breakdown of the multisig proposal', () => {
        ...
        const proposalSettings = generateMultisigPluginSettings({
            minApprovals: 4,
        ...
        expect(
            screen.getByText(proposal.metrics.totalVotes),
        ).toBeInTheDocument();
        expect(
            screen.getByText(`of ${historicalMembersCount} members`),
        ).toBeInTheDocument();
    });
});
