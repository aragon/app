...
describe('ipfs utils', () => {
    describe('cidToSrc', () => {
        it('coverts a cid with the ipfs:// format into a src', () => {
            ...
            expect(ipfsUtils.cidToSrc(processedCid)).toContain(
                `${ipfsUtils['ipfsGateway']}/ipfs/${cid}`,
            );
        });

        it('coverts a simple ipfs cid into a src', () => {
            const cid = 'qrt855';
            expect(ipfsUtils.cidToSrc(cid)).toContain(
                `${ipfsUtils['ipfsGateway']}/ipfs/${cid}`,
            );
        });
        ...
        it('adds the image size query params to the image src', () => {
            ...
            expect(src).toContain(
                `img-width=${size.toString()}&img-height=${size.toString()}`,
            );
        });
    ...
    describe('srcToUri', () => {
        ...
        it('converts a valid IPFS gateway URL with query params to an ipfs:// URI', () => {
            const src =
                'https://aragon-1.mypinata.cloud/ipfs/abc123?img-width=80&img-height=80';
            expect(ipfsUtils.srcToUri(src)).toEqual('ipfs://abc123');
        });
