...
describe('transaction utils', () => {
    ...
    describe('stringToMetadataHex', () => {
        it('parses the metadata cid to hex format', () => {
            const metadataCid =
                'QmT8PDLFQDWaAUoKw4BYziWQNVKChJY3CGi5eNpECi7ufD';
            const hexValue = '0xcid';
            toHexSpy.mockReturnValue(hexValue);
            expect(transactionUtils.stringToMetadataHex(metadataCid)).toEqual(
                hexValue,
            );
        ...
        it('parses a metadata uri to hex format', () => {
            const metadataUri =
                'ipfs://QmT8PDLFQDWaAUoKw4BYziWQNVKChJY3CGi5eNpECi7ufD';
            const hexValue = '0xuri';
            toHexSpy.mockReturnValue(hexValue);
            expect(transactionUtils.stringToMetadataHex(metadataUri)).toEqual(
                hexValue,
            );
    ...
    describe('encodeTransactionRequests', () => {
        const buildExecutorTransactionSpy = jest.spyOn(
            transactionUtils as any,
        ...
        it('returns the single transaction when having only one transaction', () => {
            ...
            const result = transactionUtils.encodeTransactionRequests(
                transactions,
        ...
        it('builds an executor transaction when having multiple transactions to execute', () => {
            ...
            expect(buildExecutorTransactionSpy).toHaveBeenCalledWith(
                transactions,
                network,
            );
    ...
    describe('buildExecutorTransaction', () => {
        ...
        it('encodes the transaction and return an executor transaction', () => {
            ...
            const expectedAddress =
                networkDefinitions[network].addresses.globalExecutor;
            encodeFunctionDataSpy.mockReturnValue(executorData);

            const multicallTransaction = transactionUtils[
                'buildExecutorTransaction'
            ](actions, network);
