...
export const WizardDetailsDialog: React.FC<IWizardDetailsDialogProps> = (
    props,
) => {
    const {
        title,
    ...
    return (
        <Dialog.Content
            className="flex flex-col gap-y-6 px-12 py-10"
            noInset={true}
        >
            <div className="flex flex-col gap-y-3">
                ...
                <Description className="font-normal text-base text-neutral-500 leading-normal">
                    {description}
                </Description>
                {infoLink && (
                ...
                {steps.map((step, index) => (
                    <div className="flex items-center gap-x-6 py-4" key={index}>
                        <IllustrationObject
                            ...
                            object={step.icon}
                        />
                        <p className="grow py-4 font-normal text-neutral-800 leading-normal">
                            {step.label}
                        </p>
                        <p className="font-normal text-base text-neutral-500 leading-normal">
                            {t('app.shared.wizardDetailsDialog.step', {
                                number: index + 1,
                            })}
                        </p>
                    </div>
