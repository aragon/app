...
describe('<Navigation.Links /> component', () => {
    ...
    it('renders the specified links', () => {
        const links = [
            {
                label: 'first-link',
                link: '/firstLink' as Route,
                icon: IconType.APP_ASSETS,
            },
            {
                label: 'second-link',
                link: '/secondLink' as Route,
        ...
        expect(
            screen.getByRole('link', { name: links[1].label }),
        ).toBeInTheDocument();
    });
    ...
    it('does not render the links marked as hidden', () => {
        const links = [
            {
                label: 'hidden-link',
                link: '/hidden' as Route,
                icon: IconType.APP_ASSETS,
                hidden: true,
            },
            {
                label: 'visible-link',
                link: '/visible' as Route,
        ...
        render(createTestComponent({ links }));
        expect(
            screen.queryByRole('link', { name: links[0].label }),
