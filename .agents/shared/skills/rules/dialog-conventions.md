...
- All dialogs used inside "basic actions", which are used in ActionComposer, must be opened with `stack: true` param, to make sure NestedActionsDialog is not closed when "child" dialog is closed. Relevant modules: `src/actions`, action components in `src/plugins`.
- **`stack: true` only works if the child closes itself by id — `close(location.id)`, never bare `close()`.** `close()` with no id clears the entire stack, so a cancel, a submit or an ESC inside the stacked child also drops the parent (`NestedActionsDialog`) and every nested action composed in it. Prefer `location.id` over the `MyDialogId.CHILD` enum: the provider already hands the dialog its own id, so nothing can drift from the definitions map. This applies to every exit path: the `Dialog.Header` `onClose`, the footer `secondaryAction`, and the handler that runs after a value is selected — declare one `const handleClose = () => close(location.id)` and reuse it. A sub-component of the dialog has no `location`, so it takes the handler as a prop (see `walletConnectActionDialogConnect`) instead of calling `close()` itself.
