...
|---|---|---|
| CMP-1 | fixed (amended by slice 7) | One `PermissionsListRow` component whose shared field components render in two responsive arrangements — the `md`+ accordion and, per the 2026-08-01 owner feedback round (AC-11), a below-`md` summary card whose Details/Condition buttons open the shared `PermissionDetailContent` (the graph edge-card component) in a dialog. Information parity preserved; no parallel component systems |
...
Slice-7 feedback round (2026-08-01, proof-first): AC-10 removed the duplicate empty-condition dash from the expanded desktop row (the Details list's `-` row is the sole empty state); AC-11 replaced the below-`md` accordion affordance with summary cards whose Details/Condition buttons (condition-gated, never disabled) open the shared `PermissionDetailContent` in a new `PermissionDetailsDialog` — per the owner's flag that the mobile list structure equals the graph view's permission detail card. Red→green: 6 failing proofs captured before implementation, 33 suites / 218 tests green after. The parallel-remediation divergence on the remote branch is explicitly the other developer's decision; no action here.
