...
|---|---|---|
| CMP-1 | fixed (amended by slice 7) | One `PermissionsListRow` component rendering two responsive arrangements over the same shared pieces: the `md`+ accordion, and — per the 2026-08-01 owner feedback round (AC-11) — a below-`md` inline detail card (permission-name header + the shared `PermissionDetailsList` shown directly + condition-gated toggle to `PermissionCondition`). Information parity preserved; no parallel component systems |
...
Slice-7 feedback round (2026-08-01, proof-first, two iterations): AC-10 removed the duplicate empty-condition dash from the expanded desktop row (the Details list's `-` row is the sole empty state; the two-column layout keeps a blank right column). AC-11 replaced the below-`md` accordion affordance with inline detail cards on the graph detail-card pattern: permission name as the card header, the existing details definition list (now the shared `permissionDetailsList.tsx`, also used by the expanded accordion) rendered directly, and a condition-gated Details/Condition toggle switching to the interior condition component. The first iteration (summary cards + drill-in dialog) was rejected by the owner and fully removed — no dialog, no summary duplication, nothing behind a click. Red→green both iterations; final: 32 suites / 216 tests green. The parallel-remediation divergence on the remote branch is explicitly the other developer's decision; no action here.

Slice-8 feedback round 2 (2026-08-01, proof-first): AC-12 — unrecognized conditions (unresolvable payload) get the no-condition breakdown treatment in the list row and graph detail card, while present-but-unregistered types keep their breakdown; AC-13 — the subplugin toggle now scopes to Where only (one folded `rowTargetsSubplugin` predicate, tooltip copy updated, who-side subplugins never hide rows nor activate the toggle); AC-14 — the list view sorts by the resolved Where label (stable, case-insensitive, presentation-only). Green: 32 suites / 221 tests. Production code across the whole remediation is net +51 lines vs the reconciled base; proof code +849 lines.
