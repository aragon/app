...
- Owner decision (2026-08-01, feedback round 2): unrecognized conditions get the same breakdown treatment as no condition (blank right side on desktop, no toggle on mobile — the details list still names them); the subplugin toggle hides rows based on **Where only**, not Who, with tooltip copy updated to match; the list view sorts by the resolved Where label (presentation-only — the graph still receives the identical unsorted row set).
...
- [x] **AC-10 — Single condition empty-state on wide layouts.** In the expanded row at `md`+, the condition heading/content render only when the row has a condition; the Details list's condition row with the `-` dash remains the sole empty-state signal. The two-column layout is preserved either way — with no condition the right column stays as blank space so the details list does not stretch across the full row. (2026-08-01 feedback round, corrected same day.)
- [x] **AC-11 — Mobile inline detail cards.** Below `md`, list rows are inline detail cards — not accordions, not dialogs, no summary duplication, nothing behind a click. Each card follows the graph detail-card pattern: the permission name as the card header, the **existing** details definition list rendered directly as the body (unchanged — the same component the expanded accordion shows), and a Details/Condition toggle at the top right that exists only when a condition exists (never disabled) and switches the body to the interior condition component. First implementation (summary cards + drill-in dialog) was rejected by the owner and removed. (2026-08-01 feedback round, corrected same day.)
- [x] **AC-12 — Unrecognized conditions carry no breakdown.** A condition whose payload cannot be resolved (label "Unrecognized condition") behaves like no condition for the breakdown affordances: blank right column on the expanded wide row, no Details/Condition toggle on the mobile card and in the graph detail card. The details list still shows the condition address and its "Unrecognized condition" label. Conditions with a present-but-unregistered type keep their breakdown (they are not labeled unrecognized). (Feedback round 2.)
- [x] **AC-13 — Subplugin toggle scopes to Where.** "Hide subplugin permissions" hides a row only when its **Where** endpoint is a subplugin; a subplugin as Who never hides a row. Tooltip copy states the Where-based rule. (Feedback round 2.)
- [x] **AC-14 — List ordered by Where.** The list view sorts rows by the resolved Where label (case-insensitive, stable). Sorting is list-view presentation only: the page still passes the identical filtered row array to both views, preserving AC-3. (Feedback round 2.)
...
7. **Owner feedback round (2026-08-01)** — AC-10 and AC-11 from the draft-PR review; proof-first per the proof-first-cycle skill. Status: Done after two iterations. The first implementation (summary cards with drill-in buttons opening a new dialog) was **rejected by the owner**: no dialog was asked for, the summary duplicated the details, and the details sat behind a click. Corrected the same day, proof-first again (red recaptured, then green): below `md` each row is an inline detail card following the graph detail-card pattern — the permission name as the card header, the existing details definition list (extracted verbatim into the shared `permissionDetailsList.tsx`, also used by the expanded accordion) rendered directly, and a Details/Condition toggle rendered only when a condition exists that switches the body to the interior condition component. The rejected dialog, its `PERMISSION_DETAILS` registry entries, and the `initialTab` addition to `PermissionDetailContent` were fully removed. On `md`+ the two-column expanded layout is preserved with a blank right column when no condition exists. Green: 32 suites / 216 tests focused; lint clean; type-check unchanged vs baseline; `git diff --check` clean. `NoConditionSlot` remains registered as the `'none'` dispatch (correct registry fallback; the empty state no longer renders a slot in the UI). Visual confirmation rides on the outstanding manual browser review.
8. **Owner feedback round 2 (2026-08-01)** — AC-12, AC-13, AC-14; proof-first. Status: Done. Red captured, then: `hasConditionBreakdown = hasCondition && !isUnrecognized` gates the toggle/panel in the list row and the graph detail card (the details list still names unrecognized conditions with their address); the subplugin filter folded `isSubpluginEndpoint` + `rowTouchesSubplugin` into one Where-only `rowTargetsSubplugin` predicate with tooltip copy updated; the list sorts by the resolved Where label (case-insensitive, stable — presentation-only, preserving AC-3's identical row array to both views). Conditions with a present-but-unregistered type keep their breakdown (their label is not "Unrecognized condition"). Green: 32 suites / 221 tests; lint clean; type-check unchanged vs baseline; `git diff --check` clean. Codebase size at this point: production code net +51 lines vs the reconciled base (+2016/−1965 across 52 files); test code net +849 lines (proof-first coverage); round 2 itself +14 production lines net.
...
| AC-10, AC-11 | proof-first component tests | `permissionsList.test.tsx`: inline card shows the details list directly with the permission-name header, the condition toggle exists only with a condition and switches to the condition component, and the expanded row keeps a single empty-condition state | yes |
