...
jest.mock('@/modules/governance/components/daoProposalList', () => ({
    DaoProposalList: (props: { children: ReactNode }) => (
        <div data-testid="proposal-list-mock">{props.children}</div>
    ),
}));

jest.mock('@/modules/governance/components/daoMemberList', () => ({
    DaoMemberList: {
        Container: (props: { children: ReactNode }) => (
            <div data-testid="member-list-mock">{props.children}</div>
        ),
    },
}));
...
jest.mock('@/modules/finance/components/assetList', () => ({
    AssetList: {
        Default: (props: { children: ReactNode }) => (
            <div data-testid="asset-list-mock">{props.children}</div>
        ),
    },
}));
...
describe('<DaoDashboardPageClient /> component', () => {
    ...
    it('renders the formatted DAO stats', () => {
        ...
        expect(
            screen.getByText(/dashboardDefaultHeader.stat.proposals/),
        ).toBeInTheDocument();
        expect(screen.getByText('2.34K')).toBeInTheDocument();

        expect(
            screen.getByText(/dashboardDefaultHeader.stat.members/),
        ).toBeInTheDocument();
        expect(screen.getByText('123')).toBeInTheDocument();

        expect(
            screen.getByText(/dashboardDefaultHeader.stat.treasury/),
        ).toBeInTheDocument();
    ...
    it('renders the DAO proposal list with a button to redirect to the proposals page when DAO has supported plugins', () => {
        ...
        const proposalPageLink = within(
            proposalList,
        ).getByRole<HTMLAnchorElement>('link', {
            name: /daoDashboardPage.main.viewAll/,
        });
        expect(proposalPageLink).toBeInTheDocument();
        expect(proposalPageLink.href).toMatch(
            new RegExp(`dao/${daoNetwork}/${daoAddress}/proposals`),
        );
    });
    ...
    it('renders the DAO asset list with a button to redirect to the asset page', () => {
        ...
        const assetPageLink = within(assetList).getByRole<HTMLAnchorElement>(
            'link',
            {
                name: /daoDashboardPage.main.viewAll/,
            },
        ...
        expect(assetPageLink.href).toMatch(
            new RegExp(`dao/${daoNetwork}/${daoAddress}/assets`),
        );
    });
    ...
    it('renders the DAO member list with a button to redirect to the members page when DAO has supported plugins', () => {
        ...
        const memberPageLink = within(memberList).getByRole<HTMLAnchorElement>(
            'link',
            {
                name: /daoDashboardPage.main.viewAll/,
            },
        ...
        expect(memberPageLink.href).toMatch(
            new RegExp(`dao/${daoNetwork}/${daoAddress}/members`),
        );
    });
    ...
    it('renders the dao information', () => {
        ...
        expect(
            screen.getByText(/daoDashboardPage.aside.details.title/),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/daoDashboardPage.aside.details.chain/),
        ).toBeInTheDocument();
        expect(
            screen.getByText(networkDefinitions[dao.network].name),
        ).toBeInTheDocument();

        expect(
            screen.getByText(/daoDashboardPage.aside.details.address/),
        ).toBeInTheDocument();
        const daoAddressLink = screen.getByRole('link', {
            name: addressUtils.truncateAddress(dao.address),
        });
        expect(daoAddressLink).toBeInTheDocument();
        expect(daoAddressLink).toHaveAttribute(
            'href',
            expect.stringMatching(dao.address),
        );

        expect(
            screen.getByText(/daoDashboardPage.aside.details.ens/),
        ).toBeInTheDocument();
        const daoEnsLink = screen.getByRole('link', {
            name: daoUtils.getDaoEns(dao),
        });
        expect(daoEnsLink).toBeInTheDocument();
        expect(daoEnsLink).toHaveAttribute(
            'href',
            expect.stringMatching(dao.address),
        );

        expect(
            screen.getByText(/daoDashboardPage.aside.details.launched/),
        ).toBeInTheDocument();
        const daoCreationLink = screen.getByRole('link', {
            name: 'December 2023',
        });
        expect(daoCreationLink).toBeInTheDocument();
        expect(daoCreationLink).toHaveAttribute(
            'href',
            expect.stringMatching(dao.transactionHash),
        );
    });
