export enum GovernanceDialogId {
    PUBLISH_PROPOSAL = 'PUBLISH_PROPOSAL',
    DUPLICATE_PROPOSAL_WARNING = 'DUPLICATE_PROPOSAL_WARNING',
    EXECUTE_ACTIONS = 'EXECUTE_ACTIONS',
    VOTE = 'VOTE',
    EXECUTE = 'EXECUTE',
    SELECT_PLUGIN = 'SELECT_PLUGIN',
    VERIFY_SMART_CONTRACT = 'VERIFY_SMART_CONTRACT',
    PERMISSION_CHECK = 'PERMISSION_CHECK',
    EXECUTE_CHECK = 'EXECUTE_CHECK',
    WALLET_CONNECT_ACTION = 'WALLET_CONNECT_ACTION',
    SIMULATE_ACTIONS = 'SIMULATE_ACTIONS',
    DELEGATE_STATEMENT_FORM = 'DELEGATE_STATEMENT_FORM',
    DELEGATE_STATEMENT_TRANSACTION = 'DELEGATE_STATEMENT_TRANSACTION',
}
