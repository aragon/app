...
import {
    CreateProposalFormSettings,
    type ICreateProposalFormSettingsProps,
} from './createProposalFormSettings';

jest.mock('@/shared/components/pluginSingleComponent', () => ({
    PluginSingleComponent: (props: { slotId: string; pluginId: string }) => (
        <div
            data-pluginids={props.pluginId}
...
describe('<CreateProposalFormSettings /> component', () => {
    ...
    const createTestComponent = (
        props?: Partial<ICreateProposalFormSettingsProps>,
    ) => {
    ...
    it('renders a plugin component with the plugin id and the dao-create-proposal-settings-form slot', () => {
        ...
        expect(pluginComponent.dataset.slotid).toEqual(
            GovernanceSlotId.GOVERNANCE_CREATE_PROPOSAL_SETTINGS_FORM,
        );
