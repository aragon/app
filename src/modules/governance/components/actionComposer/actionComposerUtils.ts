...
import {
    type ActionViewCreateComponent,
    actionViewRegistry,
...
class ActionComposerUtils {
    ...
    getDaoActions = ({
        ...
        const pluginActions = this.getDaoPluginActions(dao, targetDaoAddress);
    ...
    getDaoPluginActions = (dao?: IDao, targetDaoAddress?: string) => {
        const { plugins = [], address: mainDaoAddress } = dao ?? {};

        // Filter plugins to only include those installed on the target DAO
        const filteredPlugins = targetDaoAddress
            ? plugins.filter((plugin) => {
                  const pluginDaoAddress = plugin.daoAddress ?? mainDaoAddress;
                  return (
                      pluginDaoAddress?.toLowerCase() ===
                      targetDaoAddress.toLowerCase()
                  );
              })
            : plugins;
        ...
        const pluginGroups = pluginActions.flatMap(
            (data) => data?.groups ?? [],
        );
        const pluginComponents = pluginActions.reduce(
            (acc, data) => ({ ...acc, ...data?.components }),
            {},
        );
        ...
        const result = uniquePermissions.reduce(
            (acc, cur) => {
                const { items, group, components } =
                    actionViewRegistry.getActionsForPermissionId(
                        cur.permissionId,
                        cur.whereAddress,
                        t,
                    );
    ...
    getAllowedActionItems = (
        params: IGetAllowedActionItemsParams,
    ): IActionComposerInputItem[] => {
        const { t, allowedActions } = params;
        const nativeActionItems = this.getNativeActionItems(params).map(
            this.infoToSelectorMapper,
        );

        const actionItems: IActionComposerInputItem[] = allowedActions.map(
            (action, actionIndex) => {
                ...
                const nativeItem = nativeActionItems.find(
                    (item) =>
                        addressUtils.isAddressEqual(item.groupId, target) &&
                        item.info === selector,
                );
    ...
    getActionGroups = (
        ...
        const filteredCustomGroups = completeCustomGroups.filter(
            (customGroup) => !nativeGroupIds.has(customGroup.id),
        );
    ...
    getActionItems = (
        ...
        const completeCustomItems = this.getCustomActionItems({ t, abis }).map(
            this.infoToSelectorMapper,
        );
        ...
        const {
            nonGroupItems: nonGroupCustomItems,
            itemsByGroup: customItemsByGroup,
        } = this.groupActionItems(completeCustomItems);
        const {
            nonGroupItems: nativeNonGroupItems,
            itemsByGroup: nativeItemsByGroup,
        } = this.groupActionItems(completeNativeItems);

        const allNonGroupItems = [
            ...nonGroupCustomItems,
            ...nativeNonGroupItems,
        ];
        const finalCustomItems = this.getFinalCustomItems(
            customItemsByGroup,
            nativeItemsByGroup,
        );
        const finalNativeItems = this.getFinalNativeItems(
            nativeItemsByGroup,
            customItemsByGroup,
        );
        ...
        if (excludeActionTypes?.length) {
            return allItems.filter(
                ({ defaultValue }) =>
                    ...
                    !excludeActionTypes.includes(defaultValue.type),
            );
        }
    ...
    getDefaultActionPluginMetadataItem = (
        ...
        return {
            ...
            name: t(
                `app.governance.actionComposer.nativeItem.${ProposalActionType.METADATA_PLUGIN_UPDATE}`,
            ),
            icon: IconType.SETTINGS,
            groupId: address,
            defaultValue: this.buildDefaultActionPluginMetadata(
                plugin,
                additionalMetadata,
            ),
        };
    ...
    private getFinalCustomItems = (
        ...
        Object.entries(customItemsByGroup).reduce<IActionComposerInputItem[]>(
            (acc, [groupId, items]) => {
    ...
    private getFinalNativeItems = (
        ...
        Object.entries(nativeItemsByGroup).reduce<IActionComposerInputItem[]>(
            (acc, [groupId, items]) => {
    ...
    private getFunctionSelector = (item: IActionComposerInputItem) => {
        ...
        return (
            info ?? proposalActionUtils.actionToFunctionSelector(defaultValue)
        );
    };

    private buildCustomActionGroup = ({
        address,
        name,
    }: Pick<ISmartContractAbi, 'address' | 'name'>) => ({
        id: address,
        name,
    ...
    private getCustomActionItems = ({
        ...
        const customActionItems = abis.map((abi) => {
            ...
            const rawCalldataAction = this.buildDefaultRawCalldataAction(
                abi,
                t,
            );
        ...
        return [
            {
                ...
                name: t(
                    `app.governance.actionComposer.customItem.${ActionItemId.ADD_CONTRACT}`,
                ),
                icon: IconType.PLUS,
                alwaysVisible: true,
            },
    ...
    private buildTransferNativeAction = (
        t: TranslationFunction,
        token?: string,
    ) => ({
        id: ProposalActionType.TRANSFER,
        name: t(
            `app.governance.actionComposer.nativeItem.${ProposalActionType.TRANSFER}`,
        ),
    ...
    private buildDefaultRawCalldataAction = (
        ...
        name: t(
            `app.governance.actionComposer.customItem.${ActionItemId.RAW_CALLDATA}`,
        ),
        ...
        defaultValue: {
            ...
            inputData: {
                function: t(
                    'app.governance.actionComposer.rawCalldataFunction',
                ),
                contract: name,
                parameters: [],
            },
