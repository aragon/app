...
describe('<ProposalVotingTerminal /> component', () => {
    ...
    it('renders the plugin-specific proposal breakdown component', () => {
        const proposal = generateProposal({ pluginSubdomain: 'multisig', interfaceType: 'multisig' });
        ...
        expect(pluginComponent[0].dataset.pluginid).toEqual(proposal.interfaceType);
    });
    ...
    it('renders the plugin info and proposal settings', () => {
        ...
        const proposal = generateProposal({ settings, pluginSubdomain: 'plugin-id', interfaceType: 'plugin-id' });
