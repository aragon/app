...
import {
    type IProposalListStatsProps,
    ProposalListStats,
} from './proposalListStats';
...
describe('<ProposalListStats /> component', () => {
    ...
    const formatDateSpy = jest.spyOn(
        formatterUtilsModule.formatterUtils,
        'formatDate',
    );
    ...
    it('renders all stats with valid data and formatted relative date', () => {
        ...
        expect(
            screen.getByText(/proposalListStats.executed/),
        ).toBeInTheDocument();
        ...
        expect(
            screen.getByText(/proposalListStats.mostRecent/),
        ).toBeInTheDocument();
        expect(screen.getByText('13')).toBeInTheDocument();
        expect(
            screen.getByText(/proposalListStats.recentUnit/),
        ).toBeInTheDocument();
    });
    ...
    it('renders settings button with correct href and label', () => {
        ...
        expect(button).toHaveAttribute(
            'href',
            `/dao/ethereum-mainnet/${address}/settings#governance`,
        );
    });
});
