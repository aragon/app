...
export const DaoProposalListContainer: React.FC<IDaoProposalListContainerProps> = (props) => {
    ...
    if (processPlugins && processPlugins.length > 1) {
        const { pluginAddress: _ignored, ...queryParams } =
            (initialParams.queryParams as IGetProposalListParams['queryParams'] & {
                pluginAddress?: string;
            });

        const allInitialParams = { ...initialParams, queryParams };
        const allTabPlugin = {
            id: 'all',
