...
import { CreateProcessWizardStep, createProcessWizardSteps } from './createProcessPageDefinitions';
import { CreateProcessPageClientSteps, type ICreateProcessPageClientStepsProps } from './createProcessPageSteps';

jest.mock('react-hook-form', () => ({ __esModule: true, ...jest.requireActual('react-hook-form') }));

jest.mock('../../components/createProcessForm', () => ({
    CreateProcessForm: {
...
describe('<CreateProcessPageClientSteps /> component', () => {
    ...
    const createTestComponent = (props?: Partial<ICreateProcessPageClientStepsProps>) => {
        const completeProps: ICreateProcessPageClientStepsProps = {
            daoId: 'test',
            steps: createProcessWizardSteps,
            ...props,
        };

        return <CreateProcessPageClientSteps {...completeProps} />;
    };

    it('renders the wizard steps', () => {
        render(createTestComponent());

        Object.keys(CreateProcessWizardStep).forEach((step) => {
    ...
    it('renders the addAction step when addAction form value is set to true', () => {
        useWatchSpy.mockReturnValue(true);
        render(createTestComponent());
        expect(screen.getByTestId(CreateProcessWizardStep.ACTIONS).dataset.hidden).toEqual('false');
    });

    it('hides the addAction step when addAction form value is set to false', () => {
        useWatchSpy.mockReturnValue(false);
        render(createTestComponent());
        expect(screen.getByTestId(CreateProcessWizardStep.ACTIONS).dataset.hidden).toEqual('true');
    });
});
