...
jest.mock('../../components/proposalExecutionStatus', () => ({
    ProposalExecutionStatus: () => (
        <div data-testid="proposal-execution-status-mock" />
    ),
}));
...
describe('<DaoProposalDetailsPageClient /> component', () => {
    ...
    const useLastSimulationSpy = jest.spyOn(
    ...
    beforeEach(() => {
        useProposalSpy.mockReturnValue(
            generateReactQueryResultSuccess({ data: generateProposal() }),
        );
    ...
    const createTestComponent = (
        props?: Partial<IDaoProposalDetailsPageClientProps>,
    ) => {
    ...
    it('renders the proposal page breadcrumbs', () => {
        ...
        expect(proposalsLink.getAttribute('href')).toEqual(
            `/dao/${dao.network}/${dao.address}/proposals`,
        );
        expect(
            within(breadcrumbsContainer).getByText(proposalSlug),
        ).toBeInTheDocument();
    });
    ...
    it('uses the plugin-specific function to process and render the proposal status', () => {
        ...
        expect(
            screen.getByText(/daoProposalDetailsPage.aside.details.status/),
        ).toBeInTheDocument();
    });
    ...
    it('renders the proposal description when defined on a collapsible component', () => {
        ...
        expect(
            screen.getByText(/daoProposalDetailsPage.main.description.header/),
        ).toBeInTheDocument();
    ...
    it('renders the proposal info', () => {
        ...
        const detailsTitle = screen.getByText(
            /daoProposalDetailsPage.aside.details.title/,
        );
        ...
        expect(
            screen.getByText(/daoProposalDetailsPage.aside.details.onChainId/),
        ).toBeInTheDocument();
        expect(
            within(detailsContainer).getByText(proposal.proposalIndex),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/daoProposalDetailsPage.aside.details.id/),
        ).toBeInTheDocument();
        expect(
            within(detailsContainer).getByText(proposalSlug),
        ).toBeInTheDocument();

        expect(
            screen.getByText(/daoProposalDetailsPage.aside.details.published/),
        ).toBeInTheDocument();
        const creationBlockLink = screen.getByRole('link', {
            name: 'July 26, 2023',
        });
        expect(creationBlockLink).toBeInTheDocument();
        expect(creationBlockLink.getAttribute('href')).toEqual(
            'https://sepolia.etherscan.io/tx/0x4654',
        );

        expect(
            screen.getByText(/daoProposalDetailsPage.aside.details.creator/),
        ).toBeInTheDocument();
        const creatorLink = screen.getByRole('link', {
            name: proposal.creator.address,
        });
        expect(creatorLink).toBeInTheDocument();
        expect(creatorLink.getAttribute('href')).toEqual(
            'https://sepolia.etherscan.io/address/0x123',
        );
    });
    ...
    it('renders the proposal resources as external links', () => {
        ...
        expect(
            screen.getByText(/daoProposalDetailsPage.aside.links.title/),
        ).toBeInTheDocument();
    ...
    it('does not render the links section when proposal has no resources', () => {
        ...
        render(createTestComponent());
        expect(
            screen.queryByText(/daoProposalDetailsPage.aside.links.title/),
    ...
    describe('Action simulation behavior', () => {
        ...
        beforeEach(() => {
            useSimulateProposalSpy.mockReturnValue({
                ...
                ReturnType<typeof actionSimulationService.useSimulateProposal>
            > as ReturnType<
        ...
        it('invalidates proposal query after successful simulation', async () => {
            ...
            const simulationQueryKey =
                actionSimulationService.actionSimulationServiceKeys.lastSimulation(
                    {
                        urlParams: { proposalId: proposal.id },
                    },
                );
            const proposalQueryKey =
                governanceService.governanceServiceKeys.proposalBySlug({
