...
describe('useProposalList query', () => {
    const governanceServiceSpy = jest.spyOn(
        governanceService,
        'getProposalList',
    );
    ...
    it('fetches the proposals of the specified DAO', async () => {
        ...
        const proposalsResult = generatePaginatedResponse({
            data: [generateProposal()],
        });
        governanceServiceSpy.mockResolvedValue(proposalsResult);
        const { result } = renderHook(
            () => useProposalList({ queryParams: params }),
            { wrapper: ReactQueryWrapper },
        );
        await waitFor(() =>
            expect(result.current.data?.pages[0]).toEqual(proposalsResult),
        );
    });
});
