...
describe('proposalActionUtils', () => {
    const getSlotFunctionsSpy = jest.spyOn(
        pluginRegistryUtils,
        'getSlotFunctions',
    );
    ...
    describe('normalizeActions', () => {
        ...
        it('retrieves and triggers all plugin-specific normalization functions to normalize the proposal actions', () => {
            ...
            getSlotFunctionsSpy.mockReturnValue([
            ...
            expect(normalizationFunctionTwo).toHaveBeenCalledWith(
                expectedParams,
            );
        });
        ...
        it('triggers the default normalization function with the result of the plugin-specific normalization function', () => {
            const normalizedActions = [
                generateProposalAction({ type: '1' }),
                generateProposalAction({ type: '2' }),
            ];
    ...
    describe('normalizeDefaultAction', () => {
        ...
        it('uses the update-metadata-action normalization function when action is of update-metadata type', () => {
            ...
            normalizeUpdateMetaDataActionSpy.mockReturnValue(
                normalizedAction as IProposalActionUpdateMetadata,
            );
            expect(proposalActionUtils.normalizeDefaultAction(action)).toEqual(
                normalizedAction,
            );
            expect(normalizeUpdateMetaDataActionSpy).toHaveBeenCalledWith(
                action,
            );
        });
    ...
    describe('normalizeUpdateMetaDataAction', () => {
        const normalizeActionMetadataSpy = jest.spyOn(
            proposalActionUtils,
        ...
        it('correctly normalizes a update-dao-metadata action', () => {
            ...
            const normalizedAction =
                proposalActionUtils.normalizeUpdateMetaDataAction(action);
            expect(normalizedAction.type).toEqual(
                GukProposalActionType.UPDATE_METADATA,
            );
            expect(normalizedAction.existingMetadata).toEqual(
                normalizedMetadata,
            );
            expect(normalizedAction.proposedMetadata).toEqual(
                normalizedMetadata,
            );
        });
    ...
    describe('normalizeActionMetadata', () => {
        const normalizeActionMetadataLinksSpy = jest.spyOn(
            proposalActionUtils,
        ...
        it('sets default values for metadata when missing on the action metadata object', () => {
            const metadata = {};
            const normalizedMetadata =
                proposalActionUtils.normalizeActionMetadata(metadata);
        ...
        it('correctly normalizes DAO metadata object', () => {
            ...
            const expectedAvatarUrl =
                'https://aragon-1.mypinata.cloud/ipfs/valid-cid?img-width=80&img-height=80';
            ipfsCidToSrcSpy.mockReturnValue(expectedAvatarUrl);
            const normalizedMetadata =
                proposalActionUtils.normalizeActionMetadata(metadata);
            expect(normalizeActionMetadataLinksSpy).toHaveBeenCalledWith(
                metadata.links,
            );
    ...
    describe('normalizeActionMetadataLinks', () => {
        ...
        it('correctly normalizes the metadata links', () => {
            ...
            expect(
                proposalActionUtils.normalizeActionMetadataLinks(links),
            ).toEqual([
    ...
    describe('isUpdateMetadataAction', () => {
        it('returns true when action is of update-dao-metadata type', () => {
            ...
            expect(
                proposalActionUtils.isUpdateMetadataAction(action),
            ).toBeTruthy();
        });
    ...
    describe('normalizeActionMetadataAvatar', () => {
        it('returns the correct avatar URL when metadata has a valid IPFS avatar', () => {
            ...
            const result =
                proposalActionUtils.normalizeActionMetadataAvatar(metadata);
