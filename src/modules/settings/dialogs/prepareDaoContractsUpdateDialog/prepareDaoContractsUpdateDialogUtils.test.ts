...
describe('prepareDaoContractsUpdateDialog utils', () => {
    ...
    const getPluginUpdateSetupDataSpy = jest.spyOn(
        pluginTransactionUtils,
        'getPluginUpdateSetupData',
    );
    const buildApplyPluginsUpdateActionsSpy = jest.spyOn(
        pluginTransactionUtils,
    ...
    describe('buildPrepareUpdatePluginsTransaction', () => {
        const buildTxSpy = jest.spyOn(
            prepareDaoContractsUpdateDialogUtils as any,
            'buildPrepareUpdateTransaction',
        );
        const encodeTxSpy = jest.spyOn(
            transactionUtils,
        ...
        it('builds the transactions to update all specified plugins and encodes them to one transaction', async () => {
            ...
            const { pluginSetupProcessor: expectedTo } =
                networkDefinitions[dao.network].addresses;
            buildTxSpy
                .mockReturnValueOnce(updateTransactions[0])
                .mockReturnValueOnce(updateTransactions[1]);
            encodeTxSpy.mockReturnValue(encodedTransaction);

            const result =
                await prepareDaoContractsUpdateDialogUtils.buildPrepareUpdatePluginsTransaction(
                    {
                        dao,
                        plugins,
                    },
                );
            const expectedTransactions = updateTransactions.map((data) => ({
                to: expectedTo,
                data,
                value: BigInt(0),
            }));
            ...
            expect(encodeTxSpy).toHaveBeenCalledWith(
    ...
    describe('getApplyUpdateProposal', () => {
        const buildActionsSpy = jest.spyOn(
            prepareDaoContractsUpdateDialogUtils as any,
            'buildApplyUpdateTransactions',
        );

        const buildMetaSpy = jest.spyOn(
            prepareDaoContractsUpdateDialogUtils as any,
            'buildApplyUpdateMetadata',
        );
        ...
        it('builds and return the apply udpate proposal actions and metadata', () => {
            ...
            expect(buildActionsSpy).toHaveBeenCalledWith({
                dao,
                plugins,
                osxUpdate,
            });
            expect(buildMetaSpy).toHaveBeenCalledWith({
                dao,
                plugins,
                osxUpdate,
            });
    ...
    describe('buildPrepareUpdateTransaction', () => {
        const buildPayloadSpy = jest.spyOn(
            prepareDaoContractsUpdateDialogUtils as any,
            'buildPluginSetupPayload',
        );
        ...
        it('encodes the prepareUpdate transaction for the given plugin', async () => {
            ...
            const result = await prepareDaoContractsUpdateDialogUtils[
                'buildPrepareUpdateTransaction'
            ](dao, plugin);
            ...
            expect(encodeFunctionDataSpy).toHaveBeenCalledWith({
                ...
                args: [
                    dao.address,
                    {
                        currentVersionTag: { release: 1, build: 4 },
                        newVersionTag,
                        pluginSetupRepo,
                        setupPayload,
                    },
    ...
    describe('buildPluginSetupPayload', () => {
        ...
        it('throws error when the plugin-specific data builder function is not registered', async () => {
            ...
            const expectedMessage = expect.stringMatching(
                'builder function does not exist',
            ) as unknown;
            getSlotFunctionSpy.mockReturnValue(undefined);
            getPluginInstallationDataSpy.mockResolvedValue({
                preparedSetupData: { helpers: [] },
            });
            await expect(
                prepareDaoContractsUpdateDialogUtils['buildPluginSetupPayload'](
                    dao,
                    plugin,
                ),
            ).rejects.toEqual(
    ...
    describe('buildApplyUpdateTransactions', () => {
        const buildOsxSpy = jest.spyOn(
            prepareDaoContractsUpdateDialogUtils as any,
            'buildOsxUpdateAction',
        );
        ...
        it('builds the plugin-update transactions when the prepare tx receipt is set', () => {
            ...
            const pluginSetupData = [
                { initData: '0x1' },
                { initData: '0x2' },
            ] as unknown as IPluginUpdateSetupData[];
            ...
            buildApplyPluginsUpdateActionsSpy.mockReturnValue(
                updateTransactions,
            );
            ...
            expect(getPluginUpdateSetupDataSpy).toHaveBeenCalledWith(
                prepareUpdateReceipt,
            );
    ...
    describe('buildOsxUpdateAction', () => {
        it('builds the transaction to update the protocol version of the DAO', () => {
            ...
            encodeFunctionDataSpy
                ...
                .mockReturnValueOnce(upgradeData);

            const result =
                prepareDaoContractsUpdateDialogUtils['buildOsxUpdateAction'](
                    dao,
                );
            ...
            expect(encodeFunctionDataSpy).toHaveBeenNthCalledWith(2, {
                ...
                args: [
                    networkDefinitions[dao.network].addresses.dao,
                    initializeData,
                ],
            });
            expect(result).toEqual({
    ...
    describe('buildApplyUpdateMetadata', () => {
        const summarySpy = jest.spyOn(
            prepareDaoContractsUpdateDialogUtils as any,
            'getApplyUpdateSummaryMetadata',
        );

        const bodySpy = jest.spyOn(
            prepareDaoContractsUpdateDialogUtils as any,
            'getApplyUpdateBodyMetadata',
        );
    ...
    describe('getApplyUpdateBodyMetadata', () => {
        const osxDetailsSpy = jest.spyOn(
            prepareDaoContractsUpdateDialogUtils as any,
            'getOsxUpdateDetails',
        );

        const pluginDetailsSpy = jest.spyOn(
            prepareDaoContractsUpdateDialogUtils as any,
        ...
        it('returns the body of the update DAO contracts proposal', () => {
            ...
            pluginDetailsSpy
                .mockReturnValueOnce(pluginsUpdate[0])
                .mockReturnValueOnce(pluginsUpdate[1]);
            const result =
                prepareDaoContractsUpdateDialogUtils[
                    'getApplyUpdateBodyMetadata'
                ](params);
        ...
        it('does not includes the osx-updates details when the osxUpdate parameter is set to false', () => {
            ...
            prepareDaoContractsUpdateDialogUtils['getApplyUpdateBodyMetadata'](
                params,
            );
    ...
    describe('getOsxUpdateDetails', () => {
        it('returns the details of the osx update', () => {
            ...
            networkDefinitions[dao.network].protocolVersion.releaseNotes =
                'https://github.com/notes';
            networkDefinitions[dao.network].protocolVersion.description =
                'OSX-summary';
            const result =
                prepareDaoContractsUpdateDialogUtils['getOsxUpdateDetails'](
                    dao,
                );
