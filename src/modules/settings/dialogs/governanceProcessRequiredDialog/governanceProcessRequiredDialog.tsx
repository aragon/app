...
export const GovernanceProcessRequiredDialog: React.FC<
    IGovernanceProcessRequiredDialogProps
> = (props) => {
    const { location } = props;
    invariant(
        location.params != null,
        'GovernanceProcessRequiredDialog: required parameters must be set.',
    );
    ...
    const createProcessUrl = daoUtils.getDaoUrl(
        dao,
    ...
    const { check: createProcessGuard, result: canCreateProcess } =
    ...
    return (
        <>
            ...
            <Dialog.Content className="flex flex-col items-center gap-4">
                <EmptyState
                    description={t(
                        'app.settings.governanceProcessRequiredDialog.feedback.description',
                    )}
                    heading={t(
                        'app.settings.governanceProcessRequiredDialog.feedback.title',
                    )}
                    objectIllustration={{ object: 'USERS' }}
                    primaryButton={{
                        label: t(
                            'app.settings.governanceProcessRequiredDialog.feedback.action',
                        ),
