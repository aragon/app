...
class PrepareProcessDialogUtils {
    private publishProcessProposalMetadata = {
        ...
        summary:
            'This proposal applies the plugin installation to create the new process',
    };
    ...
    preparePluginsMetadata = (values: ICreateProcessFormData) => {
        ...
        const pluginsMetadata = newStageBodies.map((body) =>
            prepareProcessDialogUtils.preparePluginMetadata(body),
        );

        return { pluginsMetadata, processorMetadata };
    };

    buildPrepareProcessTransaction = (
        ...
        const { processor: processorMetadata, plugins: pluginsMetadata } =
            processMetadata;
        const { pluginSetupProcessor, conditionFactory } =
            networkDefinitions[dao.network].addresses;

        const deployExecuteSelectorConditionData =
            this.buildDeployExecuteSelectorConditionData({
                dao,
                permissionSelectors,
            });
        ...
        const processorInstallAction =
            processorMetadata != null
                ? this.buildPrepareInstallProcessorActionData(
                      processorMetadata,
                      dao,
                  )
                : undefined;
        ...
        const installActionsData =
            processorInstallAction != null
                ? [processorInstallAction, ...pluginInstallActions]
        ...
        const safeConditionsDeployTransactions = safeConditionsDeployData.map(
            (data) => ({
                to: conditionFactory,
                value: BigInt(0),
                data,
        ...
        const encodedTransaction = transactionUtils.encodeTransactionRequests(
            [
                ...executeConditionDeployTransaction,
                ...installActionTransactions,
                ...safeConditionsDeployTransactions,
            ],
            dao.network,
        );
    ...
    buildPublishProcessProposalActions = (
        params: IBuildProcessProposalActionsParams,
    ): ITransactionRequest[] => {
        const {
            values,
            dao,
        ...
        const processorSetupActions = isAdvancedGovernance
            ? sppTransactionUtils.buildPluginsSetupActions(
                  values,
                  setupData,
                  dao,
                  safeConditionAddresses,
        ...
        const proposalActions =
            pluginTransactionUtils.buildApplyPluginsInstallationActions(
                buildActionsParams,
            );
    ...
    preparePublishProcessProposalMetadata = () =>
        this.publishProcessProposalMetadata;

    getExecuteSelectorConditionAddress = (
    ...
    getSafeConditionAddresses = (
        ...
        return safeConditionLogs
            .map((log) => log.args.newContract)
    ...
    private buildPrepareInstallProcessorActionData = (
        metadata: string,
        dao: IDao,
    ) => {
        const processorMetadata =
            transactionUtils.stringToMetadataHex(metadata);
        const processorInstallData =
            sppTransactionUtils.buildPreparePluginInstallData(
                processorMetadata,
                dao,
            );
    ...
    private buildPrepareInstallPluginsActionData = (
        params: IBuildPrepareInstallPluginsActionParams,
    ) => {
        ...
        const newStageBodies = values.stages
            .flatMap((stage, stageIndex) =>
                stage.bodies.map((body) => ({ ...body, stageIndex })),
            )
            .filter((body) => body.type === BodyType.NEW);

        const installData = newStageBodies.map((body, index) => {
            const { votingPeriod: stageVotingPeriod } =
                values.stages[body.stageIndex].settings;
    ...
    private buildSafeConditionsDeployData = (
        params: IBuildPrepareInstallPluginsActionParams,
    ) => {
        const { values } = params;
        const { governanceType } = values;
    ...
    private buildPrepareInstallPluginActionData = (
        params: IBuildPrepareInstallPluginActionParams,
    ) => {
        ...
        const prepareFunction = pluginRegistryUtils.getSlotFunction<
    ...
    private buildDeployExecuteSelectorConditionData = (
        params: IBuildDeployExecuteSelectorConditionDataParams,
    ) => {
        const { dao, permissionSelectors } = params;

        const groupedSelectors = Object.groupBy(
            permissionSelectors,
            (selector) => selector.to,
        );
        const selectorTargets = Object.entries(groupedSelectors).map(
            ([address, actions = []]) => ({
                where: address as Hex,
                selectors: actions.map((action) =>
