...
describe('<ApplicationTags /> component', () => {
    ...
    it('renders current version and DEV label on development environment', () => {
        ...
        render(createTestComponent());
        expect(screen.getByText(/applicationTags.versionEnv \(version=1.0.2,env=DEV\)/)).toBeInTheDocument();
    });
    ...
    it('renders current version and STG label on staging environment', () => {
        ...
        render(createTestComponent());
        expect(screen.getByText(/applicationTags.versionEnv \(version=0.0.1,env=STG\)/)).toBeInTheDocument();
    });
    ...
    it('only renders current version of production environment', () => {
        ...
        render(createTestComponent());
        expect(screen.getByText(/applicationTags.version \(version=1.5.0,env=undefined\)/)).toBeInTheDocument();
    });
});
