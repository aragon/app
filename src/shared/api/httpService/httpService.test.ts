...
describe('Http service', () => {
    ...
    describe('request', () => {
        ...
        it('throws error from errorHandler property on request error', async () => {
            ...
            serviceTest = generateHttpService(undefined, () =>
                Promise.resolve(expectedError),
            );
            fetchSpy.mockResolvedValue(response);
            await expect(serviceTest.request('/test')).rejects.toEqual(
                expectedError,
            );
        });
    ...
    describe('buildUrl', () => {
        ...
        it('correctly builds the full url from the given base url and url parameters', () => {
            ...
            serviceTest = generateHttpService(baseUrl);
            expect(serviceTest['buildUrl'](url, { urlParams })).toEqual(
                expectedUrl,
            );
        });
        ...
        it('builds the full url from the given base url, query parameters and url parameters', () => {
            ...
            serviceTest = generateHttpService(baseUrl);
            expect(
                serviceTest['buildUrl'](url, { queryParams, urlParams }),
            ).toEqual(expectedUrl);
        });
    });
    ...
    describe('buildOptions', () => {
        it('appends content-type header when request type is POST and body is not FormData', () => {
            ...
            expect(processedOptions.headers.get('Content-type')).toEqual(
                'application/json',
            );
            expect(processedOptions.headers.get('key')).toEqual(
                options.headers.key,
            );
        });
    ...
    describe('parseBody', () => {
        ...
        it('returns the body as string when body is not FormData', () => {
            const body = { key: 'value' };
            expect(generateHttpService()['parseBody'](body)).toEqual(
                JSON.stringify(body),
            );
        });
    ...
    describe('replaceUrlParams', () => {
        it('returns the current url on undefined params', () => {
            const url = '/test';
            expect(serviceTest['replaceUrlParams'](url, undefined)).toEqual(
                url,
            );
        });
        ...
        it('correctly sets the url parameters on the given url', () => {
            ...
            expect(serviceTest['replaceUrlParams'](url, params)).toEqual(
                expectedUrl,
            );
        });
    ...
    describe('parseQueryParams', () => {
        it('correctly parses boolean, string, number, null and array values', () => {
            ...
            expect(serviceTest['parseQueryParams'](params)).toEqual(
                expectedResult,
            );
        });
