...
describe('versionComparator Utils', () => {
    describe('isLessThan', () => {
        it.each([
            {
                current: { release: 1, build: 2 },
                target: { release: 1, build: 3 },
                result: true,
            },
            {
                current: { release: 1, build: 3 },
                target: { release: 2, build: 0 },
                result: true,
            },
            {
                current: { release: 1, build: 3, patch: 1 },
                target: { release: 2, build: 0, patch: 2 },
                result: true,
            },
            {
                current: { release: 1, build: 3 },
            ...
            {
                current: { release: 1, build: 4 },
                target: { release: 1, build: 3 },
                result: false,
            },
            {
                current: undefined,
                target: { release: 0, build: 1 },
                result: false,
            },
            {
                current: { release: 1, build: 3 },
    ...
    describe('isGreaterThan', () => {
        it.each([
            ...
            {
                current: { release: 2, build: 0 },
                target: { release: 1, build: 3 },
                result: true,
            },
            {
                current: { release: 1, build: 3, patch: 3 },
                target: { release: 1, build: 3, patch: 1 },
                result: true,
            },
            {
                current: { release: 1, build: 3 },
    ...
    describe('normaliseComparatorInput', () => {
        it('returns undefined when input is not defined', () => {
            expect(
                versionComparatorUtils.normaliseComparatorInput(undefined),
            ).toBeUndefined();
        });
        ...
        it.each([
            ...
            expect(
                versionComparatorUtils.normaliseComparatorInput(value),
            ).toEqual(result);
        });

        it.each([
            {
                value: { release: '1', build: '2', patch: '0' },
                result: { release: 1, build: 2, patch: 0 },
            },
            {
                value: { release: '3', build: '10' },
                result: { release: 3, build: 10 },
            },
