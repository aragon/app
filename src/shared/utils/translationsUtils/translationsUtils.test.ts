...
describe('translations utils', () => {
    describe('t function', () => {
        it('returns the string associated to the given key', () => {
            ...
            const translations = {
                app: { module: { component: { key: string } } },
        ...
        it('correctly replaces the given values on the translation', () => {
            const key = 'app.key';
            const string =
                'String with values: {{firstValue}} and {{secondValue}}';
            const values = { firstValue: 'abc', secondValue: 123 };
            const translations = {
                app: { key: string },
            } as unknown as Translations;
            expect(
                translationUtils.t(translations)(key, { ...values }),
            ).toEqual(
        ...
        it('replaces all occurrences of the given value on the translation string', () => {
            ...
            expect(
                translationUtils.t(translations)(key, { ...values }),
            ).toEqual(`First ${values.value} and second ${values.value}`);
        });
        ...
        it('returns the translation key when the relative value is not found', () => {
            ...
            const translations = {
                app: { module: { key: 'value' } },
            } as unknown as Translations;
            expect(translationUtils.t(translations)(notFound)).toEqual(
                notFound,
            );
