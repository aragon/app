...
describe('buildDaoBreadcrumbPath', () => {
    it('returns undefined when no dao or target is provided', () => {
        expect(
            daoBreadcrumbsUtils.buildDaoBreadcrumbPath({
                rootDao: undefined,
    ...
    it('returns the root DAO when target matches the root', () => {
        ...
        const path = daoBreadcrumbsUtils.buildDaoBreadcrumbPath({
            rootDao: dao,
            targetAddress: '0xparent',
        });
    ...
    it('returns a deep breadcrumb chain for nested SubDAOs', () => {
        const grandChild = generateSubDao({
            address: '0xgrand',
            name: 'Grandchild DAO',
        });
