...
export interface IDaoTargetIndicatorProps {
    ...
    /**
     * Size variant affecting text size and gap.
     * - 'md': text-sm (14px), gap-2 (8px)
     * - 'sm': text-xs (12px), gap-1.5 (6px)
     * @default 'md'
     */
    size?: 'md' | 'sm';
}
...
const resolveTargetAddress = (
    ...
    if (targetDaoAddress != null) {
        return targetDaoAddress.toLowerCase();
    }
    return (plugin?.daoAddress ?? mainDaoAddress)?.toLowerCase();
};
...
export const DaoTargetIndicator: React.FC<IDaoTargetIndicatorProps> = (
    ...
    const { dao, plugin, targetDaoAddress, size = 'md' } = props;
    ...
    return (
        <div
            className={`flex items-center ${gapClass} text-neutral-500 leading-tight`}
        >
            <DaoAvatar name={targetDao.name} size="sm" src={avatarSrc} />
            <span className={`${textSizeClass} truncate`}>
                {targetDao.name}
            </span>
        </div>
    );
