...
import {
    type IWizardPageContainerProgressProps,
    WizardPageContainerProgress,
} from './wizardPageContainerProgress';
...
describe('<WizardPageContainerProgress /> component', () => {
    ...
    const createTestComponent = (
        props?: Partial<IWizardPageContainerProgressProps>,
    ) => {
    ...
    it('renders the current step, the total number of steps and current progress', async () => {
        ...
        useWizardContextSpy.mockReturnValue(
            generateWizardContext({ activeStepIndex, hasNext, steps }),
        );
        ...
        expect(
            screen.getByText(/wizardPage.container.total \(total=4\)/),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/wizardPage.container.next/),
        ).toBeInTheDocument();
        expect(
            screen.getByText(steps[activeStepIndex + 1].meta.name),
        ).toBeInTheDocument();
    ...
    it('does not render the next label when active step is the last step and finalStep property is not defined', () => {
        ...
        render(createTestComponent());
        expect(
            screen.queryByText(/wizard.container.next/),
