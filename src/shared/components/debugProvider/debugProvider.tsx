...
export const DebugContextProvider: React.FC<IDebugContextProviderProps> = (
    ...
    const registerControl = useCallback(
        (control: IDebugContextControl) => {
            ...
            setControls((current) => {
                const alreadyRegistered = current.find(
    ...
    const unregisterControl = useCallback(
        (controlName: string) =>
            setControls((current) =>
                current.filter(({ name }) => name !== controlName),
            ),
        [],
    );
    ...
    return (
        <debugContext.Provider value={contextValues}>
            {children}
...
>(): IDebugContext<TValues> => {
    ...
    if (context == null) {
        throw new Error(
            'useDebugContext: hook must be called inside a DebugContextProvider to work properly.',
        );
    }
