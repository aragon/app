...
import {
    multisigPluginAbi,
    multisigPluginPrepareUpdateAbi,
    multisigPluginSetupAbi,
} from './multisigPluginAbi';

// The end-date form values are set to "partial" because users can also create proposals without the proposal wizard
export interface ICreateMultisigProposalFormData
...
class MultisigTransactionUtils {
    buildCreateProposalData = (
        params: IBuildCreateProposalDataParams<
            ICreateMultisigProposalFormData,
    ...
    buildPrepareInstallData = (
        params: IBuildPreparePluginInstallDataParams<IMultisigSetupGovernanceForm>,
    ) => {
        ...
        const multisigTarget = pluginTransactionUtils.getPluginTargetConfig(
            dao,
