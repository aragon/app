...
export interface ICreateLockToVoteProposalFormData
...
class LockToVoteTransactionUtils {
    ...
    buildPrepareInstallData = (params: IPrepareTokenInstallDataParams) => {
        ...
        const repositoryAddress =
            lockToVotePlugin.repositoryAddresses[dao.network];
    ...
    buildCreateProposalData = (
        params: IBuildCreateProposalDataParams<
            ICreateLockToVoteProposalFormData,
    ...
    buildVoteData = (
        params: IBuildVoteDataParams<number, ILockToVoteOption>,
    ): Hex => {
        const { lockAmount } = params.vote;

        const shouldLock = lockAmount != null && lockAmount > 0;
        const data = shouldLock
            ? this.buildLockAndVoteData(params)
            : this.buildVoteDataDefault(params);

        return data;
    };

    getUninstallHelpers = (
        params: IGetUninstallHelpersParams<ILockToVotePlugin>,
    ): Hex[] => {
        const { proposalCreationConditionAddress, lockManagerAddress } =
            params.plugin;
