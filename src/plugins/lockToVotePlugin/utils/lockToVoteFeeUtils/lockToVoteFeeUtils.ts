...
class LockToVoteFeeUtils {
     ...
     */
    determineFeeMode = (ticket: ILockToVoteTicket): LockToVoteFeeMode => {
        // Fixed: minFeePercent === feePercent
        if (ticket.minFeePercent === ticket.feePercent) {
            return LockToVoteFeeMode.FIXED;
        }

        // Tiered: slope === 0
        if (ticket.slope === BigInt(0)) {
            return LockToVoteFeeMode.TIERED;
        }

        // Dynamic: linear decay
        return LockToVoteFeeMode.DYNAMIC;
    };
...
export const lockToVoteFeeUtils = new LockToVoteFeeUtils();
