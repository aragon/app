...
import { Button, Dialog, IconType, type ICompositeAddress, type IDialogRootProps } from "@aragon/gov-ui-kit";
...
export const ManageAdminsDialog: React.FC<IManageAdminsDialogProps> = (props) => {
  const {currentAdmins, pluginAddress, ...otherProps } = props;
  ...
  const {fields: adminMembersField, append: addMember, remove: removeMember} = useFieldArray({ name: membersFieldName, control });

  const handleAddMember = () => addMember({ address: '' });

  const handleRemoveMember = (index: number) => {
    if (adminMembersField.length > 1) {
      removeMember(index);
    }
  };
  ...
  console.log('adminMembersField', adminMembersField);
  ...
  return (
      <Dialog.Root {...otherProps}>
        ...
        <Dialog.Content description={t('app.plugins.admin.manageAdminsDialog.description')}>
          ...
          <form className="flex w-full flex-col gap-3 md:gap-2" onSubmit={handleSubmit(handleFormSubmit)}>
            {adminMembersField.map((field, index) => (
              ...
              onRemoveMember={() => handleRemoveMember(index)}
