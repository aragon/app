...
describe('SppStageUtils', () => {
    ...
    describe('canAdvanceStage', () => {
        ...
        it('returns true when approval is reached, minAdvance is in the past, and proposal is not a signaling proposal', () => {
            const now = '2023-01-01T12:00:00.000Z';
            const minAdvance = 3600;
            const stageStartDate = DateTime.fromISO(now).minus({ hours: 2 });
            const stage = generateSppStage({ minAdvance, stageIndex: 0 });
            const proposal = generateSppProposal({
                actions: [generateProposalAction()],
                settings: generateSppPluginSettings({ stages: [stage, generateSppStage()] }),
            });

            getStageStartDateSpy.mockReturnValue(stageStartDate);
            isApprovalReachedSpy.mockReturnValue(true);
            timeUtils.setTime(now);

            const result = sppStageUtils.canAdvanceStage(proposal, stage);
        ...
        it('returns false when minAdvance is in the future', () => {
            const now = '2023-01-01T12:00:00.000Z';
            const minAdvance = 7200;
            const stageStartDate = DateTime.fromISO(now).minus({ hours: 1 });
            const stage = generateSppStage({ minAdvance, stageIndex: 0 });
            const proposal = generateSppProposal({
                actions: [generateProposalAction()],
                settings: generateSppPluginSettings({ stages: [stage, generateSppStage()] }),
        ...
        it('returns false when proposal is a signaling proposal (no actions and last stage)', () => {
            const now = '2023-01-01T12:00:00.000Z';
            const minAdvance = 3600;
            const stageStartDate = DateTime.fromISO(now).minus({ hours: 2 });
            const stage = generateSppStage({ minAdvance, stageIndex: 1 });
            const proposal = generateSppProposal({
                actions: [],
                settings: generateSppPluginSettings({ stages: [generateSppStage(), stage] }),
            });
        ...
        it('returns true when proposal has no actions but stage is not the last one', () => {
            const now = '2023-01-01T12:00:00.000Z';
            const minAdvance = 3600;
            const stageStartDate = DateTime.fromISO(now).minus({ hours: 2 });
            const stage = generateSppStage({ minAdvance, stageIndex: 0 });
        ...
        it('returns true when minAdvance is zero and approval is reached', () => {
            const now = '2023-01-01T12:00:00.000Z';
            const minAdvance = 0;
            const stageStartDate = DateTime.fromISO(now).minus({ minutes: 10 });
            const stage = generateSppStage({ minAdvance, stageIndex: 0 });
            const proposal = generateSppProposal({
                actions: [generateProposalAction()],
                settings: generateSppPluginSettings({ stages: [stage, generateSppStage()] }),
        ...
        it('returns false when minAdvance is zero but approval is not reached', () => {
            const now = '2023-01-01T12:00:00.000Z';
            const minAdvance = 0;
            const stageStartDate = DateTime.fromISO(now).minus({ minutes: 10 });
            const stage = generateSppStage({ minAdvance, stageIndex: 0 });
