...
export interface ITokenApproveTokensDialogProps
    extends IDialogComponentProps<ITokenApproveTokensDialogParams> {}

export const TokenApproveTokensDialog: React.FC<
    ITokenApproveTokensDialogProps
> = (props) => {
    const { location } = props;
    invariant(
        location.params != null,
        'TokenApproveTokensDialog: required parameters must be set.',
    );
    const {
        token,
        amount,
    ...
    return (
        <TransactionDialog
            ...
            stepper={stepper}
            submitLabel={t(
                `app.plugins.token.tokenApproveTokensDialog.${translationNamespace}.submit`,
            )}
            title={t(
                `app.plugins.token.tokenApproveTokensDialog.${translationNamespace}.title`,
            )}
