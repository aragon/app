...
describe('tokenSettings utils', () => {
    describe('ratioToPercentage', () => {
        it('correctly parses the percentage setting', () => {
            ...
            expect(tokenSettingsUtils.ratioToPercentage(123_456)).toEqual(
                12.3456,
            );
            expect(tokenSettingsUtils.ratioToPercentage(0)).toEqual(0);
            expect(tokenSettingsUtils.ratioToPercentage(1_000_000)).toEqual(
                100,
            );
        });
    });
    ...
    describe('percentageToRatio', () => {
        it('correctly converts percentage to ratio', () => {
            ...
            expect(tokenSettingsUtils.percentageToRatio(12.3456)).toEqual(
                123_456,
            );
            expect(tokenSettingsUtils.percentageToRatio(0)).toEqual(0);
            expect(tokenSettingsUtils.percentageToRatio(100)).toEqual(
                1_000_000,
            );
        });
    });
    ...
    describe('parseSettings', () => {
        it('correctly formats and displays the approval threshold', () => {
            ...
            expect(approvalThresholdTerm.term).toMatch(
                /tokenGovernanceSettings.approvalThreshold/,
            );
            expect(approvalThresholdTerm.definition).toMatch(
                /tokenGovernanceSettings.threshold \(threshold=30%\)/,
            );
        });
        ...
        it('correctly formats and displays the minimum participation', () => {
            ...
            expect(minimumParticipationTerm.term).toMatch(
                /tokenGovernanceSettings.minimumParticipation/,
            );
        ...
        it('correctly formats and displays the duration from settings', () => {
            ...
            expect(durationTerm.term).toMatch(
                /tokenGovernanceSettings.minimumDuration/,
            );
            expect(durationTerm.definition).toMatch(
                /tokenGovernanceSettings.duration \(days=7,hours=0,minutes=0\)/,
            );
        });
        ...
        it('correctly formats and displays the voting power necessary to be a proposer', () => {
            ...
            expect(proposerVotingTerm.term).toMatch(
                /tokenGovernanceSettings.proposalThreshold/,
            );
        ...
        it('correctly formats and displays for different voting modes', () => {
            const votingModes = [
                {
                    ...
                    expectedVoteChange:
                        'app.plugins.token.tokenGovernanceSettings.yes',
                    expectedEarlyExecution:
                        'app.plugins.token.tokenGovernanceSettings.no',
                },
                {
            ...
            votingModes.forEach(
                ({ mode, expectedVoteChange, expectedEarlyExecution }) => {
                    const settings = generateTokenPluginSettings({
                        votingMode: mode,
                    });
                    const result = tokenSettingsUtils.parseSettings({
                    ...
                    expect(voteChangeTerm.definition).toEqual(
                        expectedVoteChange,
                    );
                    expect(earlyExecutionTerm.definition).toEqual(
                        expectedEarlyExecution,
                    );
                },
            );
