...
import { TokenCreateProposalSettingsForm, type ITokenCreateProposalSettingsFormProps } from './tokenCreateProposalSettingsForm';
...
jest.mock('@/shared/components/advancedDateInput', () => ({
  AdvancedDateInput: ({ label, helpText, field, infoText, isStartField, useDuration, startTime, minDuration }: IAdvancedDateInputProps) => (
...
describe('<TokenCreateProposalSettingsForm /> component', () => {
  ...
  it('passes correct props to start time AdvancedDateInput', () => {
    ...
    expect(startTimeInput).toHaveTextContent('Help Text: app.plugins.token.createProposalSettingsForm.startTime.helpText');
  ...
  it('passes correct props to end time AdvancedDateInput', () => {
    ...
    expect(endTimeInput).toHaveTextContent('Help Text: app.plugins.token.createProposalSettingsForm.endTime.helpText');
    expect(endTimeInput).toHaveTextContent('Field: app.plugins.token.createProposalSettingsForm.endTime.field');
    expect(endTimeInput).toHaveTextContent('Info Text: app.plugins.token.createProposalSettingsForm.endTime.infoText (days=0,hours=1,minutes=0)');
  ...
  it('fetches the correct min duration from the dao settings', () => {
    ...
    const daoSettings = generateDaoTokenSettings({ ...baseSettings, settings: { ...baseSettings.settings, minDuration: 3600 } });
