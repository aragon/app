...
describe('<TokenCreateProposalSettingsForm /> component', () => {
    const secondsToDaysHoursMinutesSpy = jest.spyOn(
        dateUtils,
        'secondsToDuration',
    );
    ...
    beforeEach(() => {
        secondsToDaysHoursMinutesSpy.mockReturnValue({
            days: 0,
            hours: 1,
            minutes: 0,
        });
    ...
    const createTestComponent = (
        props?: Partial<ITokenCreateProposalSettingsFormProps>,
    ) => {
    ...
    it('passes correct props to start time AdvancedDateInput', () => {
        ...
        expect(startTimeInput).toHaveTextContent(
            /tokenCreateProposalSettingsForm.startTime.label/,
        );
    ...
    it('passes correct props to end time AdvancedDateInput', () => {
        ...
        expect(endTimeInput).toHaveTextContent(
            /tokenCreateProposalSettingsForm.endTime.label/,
        );
        ...
        expect(endTimeInput).toHaveTextContent(
            'Min Duration: {"days":0,"hours":1,"minutes":0}',
        );
    });

    it('uses the correct min duration from the plugin settings', () => {
        const pluginSettings = generateTokenPluginSettings({
            minDuration: 3600,
        });
