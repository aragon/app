...
describe('<TokenMemberInfo /> component', () => {
    ...
    it('renders the component with the correct eligible voters and members info', () => {
        ...
        const plugin = generateDaoPlugin({
            settings: generateTokenPluginSettings({ token }),
        });

        render(createTestComponent({ plugin }));

        expect(
            screen.getByText(/tokenMemberInfo.eligibleVoters/),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/tokenMemberInfo.tokenHolders/),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/tokenMemberInfo.tokenLabel/),
        ).toBeInTheDocument();
        expect(
            screen.getByText(
                /tokenNameAndSymbol \(tokenName=Bitcoin,tokenSymbol=BTC/,
            ),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/tokenMemberInfo.distribution/),
        ).toBeInTheDocument();
        expect(screen.getByText(/tokenMemberInfo.supply/)).toBeInTheDocument();
        expect(
            screen.getByText(
                /tokenMemberInfo.tokenSupply \(supply=300,symbol=BTC\)/,
            ),
        ).toBeInTheDocument();
    });
    ...
    it('contains a link to the members page', () => {
        ...
        expect(linkElement).toHaveAttribute(
            'href',
            '/dao/ethereum-mainnet/0x12345/members',
        );
    });
});
