...
const useGaugeVoteDisplay = (params: {
    isUserConnected: boolean;
    isUserVotesLoading: boolean;
    formattedUserVotes: string;
    t: ReturnType<typeof useTranslations>['t'];
}) => {
    const { isUserConnected, isUserVotesLoading, formattedUserVotes, t } = params;
    if (!isUserConnected) {
        return '-';
    }
    ...
    if (formattedUserVotes === '0') {
        return t('app.plugins.gaugeVoter.gaugeVoterGaugeList.item.noVotes');
    }
    return formattedUserVotes;
};

const useGaugePercentage = (totalEpochVotingPower: number | undefined, totalVotesValue: number) => {
...
const useActionTranslationKey = (isVoted: boolean, isSelected: boolean) => {
    if (isVoted) {
        return 'voted';
    }
    if (isSelected) {
...
export const GaugeVoterGaugeListItemStructure: React.FC<IGaugeVoterGaugeListItemStructureProps> = (props) => {
    ...
    const displayUserVotes = useGaugeVoteDisplay({ isUserConnected, isUserVotesLoading, formattedUserVotes, t });
    const { formattedPercentage } = useGaugePercentage(totalEpochVotingPower, totalVotesValue);
    ...
    const actionButtonTranslationKey = useActionTranslationKey(isVoted, isSelected);
        ...
        <DataList.Item className={itemClassName} onClick={() => onViewDetails?.(gauge)}>
            ...
            <div className="flex w-full items-center justify-end md:w-36">
                <Button
                    ...
                    iconLeft={isVoted || isSelected ? IconType.CHECKMARK : undefined}
