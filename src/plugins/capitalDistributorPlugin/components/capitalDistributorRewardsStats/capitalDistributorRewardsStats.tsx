...
export const CapitalDistributorRewardsStats: React.FC<
    ICapitalDistributorRewardsStatsProps
> = (props) => {
    ...
    const { data: claimableCampaignData } = useCampaignList(
        claimableCampaignParams,
    ...
    const { blockTimestamp: latestClaimTimestamp } =
        claimedCampaigns?.pages[0].data[0]?.userData.claims[0] ?? {};
    const latestClaimDate = latestClaimTimestamp
        ? latestClaimTimestamp * 1000
        : undefined;
    ...
    const [claimDateValue, claimDateUnit] = formattedClaimDate?.split(' ') ?? [
        undefined,
        undefined,
    ];
    ...
    const stats = [
        {
            ...
            label: t(
                'app.plugins.capitalDistributor.capitalDistributorRewardsAside.stats.claimableCount',
            ),
        },
        {
            ...
            label: t(
                'app.plugins.capitalDistributor.capitalDistributorRewardsAside.stats.latestClaim',
            ),
        },
    ];
