...
import { KatanaRewardsDialogId } from '../../constants/katanaRewardsDialogId';
import { katanaRewardsMinEpochId } from '../../constants/katanaRewardsMinEpochId';
import { rewardUtils } from '../../utils/rewardUtils';

export interface IKatanaRewardsMembersFileDownloadDialogParams {
    /**
     * Gauge plugin.
     */
...
export interface IKatanaRewardsMembersFileDownloadDialogProps
    extends IDialogComponentProps<IKatanaRewardsMembersFileDownloadDialogParams> {}

export const KatanaRewardsMembersFileDownloadDialog: React.FC<
    IKatanaRewardsMembersFileDownloadDialogProps
> = (props) => {
    const { location } = props;

    invariant(
        location.params != null,
        'KatanaRewardsMembersFileDownloadDialog: params must be defined',
    );
    ...
    const validEpochs = useMemo(() => {
        ...
        const firstEpoch =
            katanaRewardsMinEpochId > currentEpochId
                ? 1
    ...
    const handleClose = () => {
        close(KatanaRewardsDialogId.KATANA_REWARDS_MEMBERS_FILE_DOWNLOAD);
    };
