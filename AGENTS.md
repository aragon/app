...
- `platform-doc/` — the product knowledge base, a squashed git subtree of `aragon/platform-doc` (branch `development`), not a workspace. The assistant builds its documentation index from it (`apps/assistant/README.md`, "Documentation answering"). Until the standalone repository is archived it is read-only here: edit upstream and sync with `git fetch https://github.com/aragon/platform-doc.git development && git rm -r -q platform-doc && git checkout HEAD -- .gitmodules && git read-tree --prefix=platform-doc/ -u FETCH_HEAD`, then commit (not `git subtree pull`: the squash commit it creates is unsigned and the repository rules refuse it). Its `protocol-doc/` is an uninitialized submodule (root `.gitmodules`) that nothing in the build reads.
